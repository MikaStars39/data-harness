...
BASE_DIR="/jpfs/qingyu/data/v32_two_stage/stage2"
SHARDS_DIR="$BASE_DIR/shards"
SHARDS_OUT_DIR="$BASE_DIR/shards_out"
LOG_DIR="/jpfs/qingyu/data-harness/output/stage2_logs"
...
echo "Stage2 dispatch on $NUM_PODS pods..."

for ((i=0; i<$NUM_PODS; i++)); do
    POD=${ALL_PODS[$i]}
    SHARD_ID=$(printf "%05d" $i)
    LOG_FILE="$LOG_DIR/stage2_${POD}_shard_${SHARD_ID}.log"
    touch "$LOG_FILE"
    ...
    (
        ...
        kubectl exec -n $NAMESPACE $POD -- bash -c "python /jpfs/qingyu/data-harness/recipe/thinking_to_cot/stage2_run_shard.py \
...
echo "Monitor: tail -f $LOG_DIR/stage2_*.log"
