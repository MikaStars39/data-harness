...
class BaseSGLangEngine:
    ...
    async def __aenter__(self):
        ...
        try:
            ...
            # Keep compatibility with older SGLang versions that do not expose this flag.
            if "enable_dp_attention" in self.engine_args and "enable_dp_attention" in str(exc):
                logger.warning("Current SGLang does not support enable_dp_attention; fallback to default attention mode.")
                retry_engine_args = dict(self.engine_args)
                retry_engine_args.pop("enable_dp_attention", None)
                self.llm = sgl.Engine(**retry_engine_args, **self.spec_args)
            else:
                raise
